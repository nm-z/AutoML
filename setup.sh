...
install_env_tpa_deps() {
    ...
    if [[ "$PYTHON_CMD" == *"3.11"* ]]; then
        log_warning "Skipping Auto-Sklearn installation for Python 3.11"
...
install_env_as_deps() {
    ...
    if [[ "$PYTHON_CMD" == *"3.11"* ]]; then
        log_warning "Auto-Sklearn 0.15.0 is incompatible with Python $PYTHON_CMD; installing base stack only"
